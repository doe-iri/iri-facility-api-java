...
public class EventMapping implements Mapping {
    ...
    public static final Map<String, Mapping> RELATIONSHIP_TARGETS = Map.of(
        Relationships.GENERATED_BY, new IncidentMapping(),
        Relationships.IMPACTS, new ResourceMapping());
    ...
    private static final Pattern PATTERN_UUID =
        Pattern.compile("/events/([0-9a-fA-F\\-]{36})$");

    /**
     * Extracts the UUID from any string containing “/events/{uuid}” at the end.
     *
     * @param url the full path or URL
     * @return the UUID that followed “/events/”
     * @throws IllegalArgumentException if no “/events/{uuid}” is found or UUID is malformed
     */
    private static UUID extractId(String url) throws IllegalArgumentException {
        ...
        throw new IllegalArgumentException("Invalid path (must end with /events/{uuid}): " + url);
    }
}
