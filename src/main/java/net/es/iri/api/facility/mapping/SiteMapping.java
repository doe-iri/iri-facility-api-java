...
public class SiteMapping implements Mapping {
    ...
    // this regex looks for “/sites/” followed by a 36-char UUID and anchors at end-of-string.
    private static final Pattern PATTERN_UUID =
        Pattern.compile("/sites/([0-9a-fA-F\\-]{36})$");

    /**
     * Extracts the UUID from any string containing “/sites/{uuid}” at the end.
     *
     * @param url the full path or URL
     * @return the UUID that followed “/sites/”
     * @throws IllegalArgumentException if no “/sites/{uuid}” is found or UUID is malformed
     */
    private static UUID extractId(String url) throws IllegalArgumentException {
        ...
        throw new IllegalArgumentException("Invalid path (must end with /sites/{uuid}): " + url);
    }
}
